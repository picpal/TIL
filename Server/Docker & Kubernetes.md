...
- 이 때문에 불필요하게 실행되는 레이어(명령어) 들이 있는데 이를 최적화 하기 위해 순서를 변경하여 실행할 수 있다. ex) npm install 은 의존서부여가 되었을 때만 재실행이 필요. 하지만 COPY 명령어 뒤에 RUN : npm install 이 있게되면 프로젝트 파일이 변경될 때 마다 npm install 